package vk.gui
{
  ...
  public class ScrollBar extends Sprite
  {
	...
	public var theme:Boolean=false;
    public static var th_col:int=0x888888;
	public static var th_sel_col:int=0xaaaaaa;
	public static var th_b_col:int=0;
	public static var th_sel_b_col:int=0;
	
    private static const SLIDER_POS_Y:uint = 18;
    private static const SLIDER_MIN_LEN:uint = 10;
    private static const LINE_POS_X:uint = 12;
    private static const LINE_POS_Y:uint = 8;
    public static const W:uint = 16;
    internal static const MIN_H:uint = 33 + SLIDER_MIN_LEN;

    public function ScrollBar( x:int, y:int, h:int, sw:int,sh:int, use_theme:Boolean=false )
    {
      this.x = x;
      this.y = y;
      this.h = h;
      ...
      trasp_bg = new Sprite();
      Utils.fillRect( trasp_bg, 0, 0, sw, sh, 0xffff00, 0.0 );
      ...
      // Scroll arrows
      arrow1 = new ScrollButton( 0, 0, false,use_theme);
      addChild( arrow1 );
      arrow1.addEventListener( Event.SCROLL, onUpScroll );

      arrow2 = new ScrollButton( 0, h - 16,true,use_theme);
      addChild( arrow2 );
    ...
    public function init( maxScrollPos:Number, pageSize:Number ):void
    {
	   ...
	   slider.y = SLIDER_POS_Y + Math.max(0,Math.min( sliderMaxY, Math.max( 0, slider.y - SLIDER_POS_Y ) ));
    }
    ...
    private function redrawSlider():void
    {
      ...
      if(!theme)Utils.rect( this, 0, 0, W, h, 0xFAFAFA, 0xcccccc );
	  ...
	  {
		  var c: uint = (isOver || dragging) ? th_sel_col : th_col;
		  var bc:uint = (isOver || dragging) ? th_sel_b_col : th_b_col;
		  
		  //Utils.fillRRect( slider, 0, 0, W, sliderLen-1, c, 5 );
		  Utils.rect( slider, 0, 0, W, sliderLen, c, bc );
	  }
    }
    ...
    private function onSliderDown( e:MouseEvent ):void
    {
      ...
      dragY = e.stageY - slider.y;
	   ...
	   var cur_pos:Point = this.localToGlobal(new Point(0, 0));
	  trasp_bg.x = -cur_pos.x;
	  trasp_bg.y = -cur_pos.y;
      addChild( trasp_bg );
